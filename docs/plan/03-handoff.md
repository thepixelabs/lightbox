...
1. **Execute an epic** with sequential phase agents: split the epic spec's §"Ordered task breakdown" into 4–8 phases; one staff-engineer agent per phase, each instructed to: read the spec + architecture + existing code; adapt to current crate versions (record every deviation in `epics/<EPIC>-deviations.md`); exit bar = build+test+clippy+fmt+deny all green; commit per phase (`"<EPIC> Phase <n>: <title>"`); never commit broken; return status done/blocked.
