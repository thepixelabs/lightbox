...
1. Read **`docs/plan/03-handoff.md`** — the single source of truth for project state: what's built, what's specced, the epic board with dependencies, and the working method. Do NOT try to read the whole `docs/` tree.
...
3. Work the next item per the handoff's **§5 working method** (phase agents with green-build gates for epics; mandate→architect→CTO governance loop for scope changes).

**Reading order for an epic:** the one `docs/plan/epics/E<nn>-*.md` spec you're executing → the `01-architecture.md` sections it cites → existing code in `crates/`. Ignore files bannered `SUPERSEDED`; ignore `docs/research/` (historical source material) unless the spec cites it.
