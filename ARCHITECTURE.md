...
This is a map of the workspace and the handful of decisions that shape everything built on top of them. For the full reasoning behind the technology choices (why wgpu over Vulkan, why SQLite, why an out-of-process ML host), see `docs/plan/01-architecture.md`, which is written ADR-style with the alternatives and reversal triggers for each choice. This document is the shorter, code-facing version: what's where, and the load-bearing decisions a new contributor needs before touching any of it.
...
- **A new migration** goes in `crates/lightbox-catalog/migrations/`, as the next sequential `NNNN_<name>.sql` file, with its number reserved first in `docs/plan/migrations.md` (the source of truth `cargo xtask lint-migrations` checks against). Migrations are forward-only and run inside one transaction each; there are no down-migrations, so a mistake is fixed by a following migration, not by editing history.
