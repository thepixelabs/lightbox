...
pub trait RenderNode: Send + Sync {
    ...
    /// Output extent given the node's **real** input extents and the render
    /// request's own target extent, identity by default; crop/rotate (E11.4)
    /// and the scale-ladder resize override.
    ///
    /// `inputs` are the extents of the tiles this node will actually be handed
    /// (`exec::Executor::evaluate` derives them from the upstream tiles, not
    /// from a request-seeded guess), so the default, pass the first input's
    /// extent through, is the truth for every pointwise node. That matters:
    /// **every** pointwise kernel, CPU and GPU, reads `input[x, y]` over its
    /// own *output* extent, so a node whose declared output extent is smaller
    /// than its input silently renders a top-left **crop** of that input
    /// rather than a resample of it (the owner-reported "moved Tint and it
    /// zoomed in on the top left corner": `global.wb` is spliced before
    /// `util.resize`, so it was the one develop node ever handed a
    /// full-size source tile while sized to the fit viewport). Overriding
    /// this to shrink an extent is therefore a promise that the node
    /// genuinely resamples, `util.resize` does; a pointwise node must not.
    ///
    /// `request` is the extent the caller asked the *pipeline* to land on
    /// (`RenderRequest::roi`'s extent). Only the scale-ladder node consumes
    /// it; it is also the fallback for a node with no inputs at all.
    fn output_extent(&self, inputs: &[Extent], params: &ParamBlock, request: Extent) -> Extent {
        let _ = params;
        inputs.first().copied().unwrap_or(request)
    }
