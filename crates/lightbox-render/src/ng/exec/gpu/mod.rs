...
impl Backend for GpuBackend {
    fn eval_node(&self, req: BackendEvalRequest<'_>) -> Result<TileHandle, RenderError> {
        ...
        // Input extents + read-only views for the node.
        let input_extents: Vec<Extent> = req
            .inputs
            .iter()
            .map(|h| h.extent().unwrap_or(Extent { w: 0, h: 0 }))
            .collect();
        ...
        // Size + acquire the output tile from the node's output port type.
        let out_extent = node.output_extent(&input_extents, req.params);
