...
///
/// This planner reasons about the graph at its **source** resolution, it maps
/// the tile grid and apron requirements over the full image, before any
/// scale-ladder decimation, so `source_extent` is what it passes as
/// [`RenderNode::output_extent`]'s `request` too. That keeps `util.resize`
/// extent-identity here, which is exactly the frame the ROI plan is expressed
/// in. The live single-tile walk ([`super::Executor::evaluate`]) derives its
/// extents from the real tiles instead, and passes the *request's* extent.
///
/// [`RenderNode::output_extent`]: crate::ng::node::RenderNode::output_extent
    ...
    for idx in topo {
        ...
        let ext = if input_idxs.is_empty() {
            ...
            let in_extents: Vec<Extent> = input_idxs
                ...
                .collect();
            node.output_extent(&in_extents, graph.params(idx), source_extent)
        };
