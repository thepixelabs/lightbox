...
impl Executor {
    ...
    pub fn evaluate(
        ...
        // The extent the caller asked the *pipeline* to land on. Only the
        // scale-ladder node (`util.resize`, via its `output_extent` override)
        // consumes it; it is also the fallback for a node with no inputs.
        let base_extent = Extent {
            w: roi.w.max(1),
            h: roi.h.max(1),
        };
        // Each node's target output extent, derived **from the real upstream
        // tiles** as the walk produces them (task E11's `output_extent` seam;
        // see `docs/plan/epics/E11-deviations.md` E-scope-6/9).
        //
        // This used to be a separate forward pass seeded from `roi` for every
        // zero-graph-input node, the source stage included, even though the
        // source stage's own output tile is the full-size decoded source and
        // never `roi`-sized. Any node spliced between `src.decoded` and
        // `util.resize` was therefore sized to the *request* while being
        // handed a *source*-extent tile, and since every pointwise kernel
        // reads `input[x, y]` over its own output extent, it rendered a
        // top-left crop of the source instead of a resample of it. The PV1
        // template splices exactly one such segment, WB, which is why
        // moving Temp/Tint "zoomed into the top left corner" while no other
        // slider did (`tests/fit_render_extent.rs`).
        //
        // Deriving from the tiles the walk actually holds keeps the two
        // extents in agreement by construction: there is no second, guessed
        // answer left to disagree with the first.
        let mut target_extents: HashMap<NodeIndex, Extent> = HashMap::with_capacity(topo.len());
        ...
        for idx in topo {
            ...
            // This node's target output extent, from the tiles it will
            // actually be handed. The source stage is sized to whatever the
            // engine injected (the real decoded/uploaded content); every other
            // node declares its own via `output_extent`, whose default passes
            // its input's extent straight through, so a pointwise node is
            // sized to its input by construction and can no longer be asked to
            // crop. `util.resize` overrides it to land on `base_extent`, which
            // is where the fit ladder's decimation actually happens.
            let target_extent = if is_source {
                source
                    .as_ref()
                    .and_then(|s| s.tile.extent())
                    .unwrap_or(base_extent)
            } else {
                let in_exts: Vec<Extent> = graph
                    .inputs_of(idx)
                    .iter()
                    .zip(&input_tiles)
                    .map(|(&src, tile)| {
                        // The tile's own extent is the truth; the recorded
                        // target is the fallback for a handle that cannot
                        // report one (it is what the backend was asked for).
                        tile.extent()
                            .or_else(|| target_extents.get(&src).copied())
                            .unwrap_or(base_extent)
                    })
                    .collect();
                node.output_extent(&in_exts, params, base_extent)
            };
            target_extents.insert(idx, target_extent);

            // Derive this node's content key and record it for downstream nodes.
            // `content_extent` is THIS node's target extent, folding it in is
            // what stops two `Fit` requests for the same image+recipe at
            // different viewport sizes from colliding on an identical key
            // (`scale_q` alone is a constant 64 for every `Fit`/`OneToOne`
            // request, regardless of the actual viewport, see
            // `CacheKeyInputs::content_extent`'s own doc). Nodes upstream of
            // `util.resize` now derive a viewport-*independent* extent, which
            // is not a weakening of that fix but a consequence of it being
            // true: their output genuinely no longer depends on the viewport,
            // so it is safe, and cheap, to reuse across one.
