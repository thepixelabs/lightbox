...
use crate::ng::types::{Extent, NodeId, ProcessVersion, TileCoord, TilePrecision};
...
pub struct CacheKeyInputs<'a> {
    ...
    pub scale_q: u16,
    /// **This request's** literal output pixel extent for the node (its
    /// propagated `target_extent`, see `exec::Executor::evaluate`'s
    /// `target_extents` walk). `RenderScale::Fit`/`OneToOne` both quantize
    /// `scale_q` to a CONSTANT 64 regardless of the actual requested
    /// viewport (`exec::scale_ratio`'s documented simplification) and no
    /// other field here is sensitive to the caller's viewport size, so
    /// without this field, two `Fit` requests for the SAME image+recipe at
    /// DIFFERENT viewport sizes (e.g. a window resize, or the app's own
    /// first-frame layout not yet settled) derive IDENTICAL keys for every
    /// node whose own params don't happen to depend on the viewport
    /// (`src.decoded`, `util.resize`, any identity-elided segment), a
    /// later, correctly-sized request silently gets served the earlier
    /// request's STALE, differently-shaped cached tile instead of
    /// re-rendering (this crate's own regression: `content_extent_flips_
    /// the_key` below, plus `engine.rs`'s `distinct_viewports_do_not_
    /// collide_on_a_stale_cached_extent`). The engine-owned SOURCE stage's
    /// own key (`Engine::source_key`) deliberately passes a CONSTANT here
    /// instead, it stays scale-independent by design (task B4: the raw
    /// decode is never re-fetched just because the viewport changed).
...
impl CacheKey {
    ...
    pub fn derive(inputs: &CacheKeyInputs<'_>) -> CacheKey {
        ...
        h.update(&[0x06]);
        // content extent, see the field's own doc (viewport-collision fix).
        h.update(&inputs.content_extent.w.to_le_bytes());
        h.update(&inputs.content_extent.h.to_le_bytes());
        CacheKey(h.finalize())
    }
}
...
mod tests {
    ...
    fn inputs(node: &'static str, salt: &[u8], params: &[u8], upstream: &[CacheKey]) -> CacheKey {
        inputs_at(node, salt, params, upstream, Extent { w: 64, h: 64 })
    }
    ...
    /// **Viewport-collision regression (root cause of the "WB edit collapses
    /// the zoom/crop" bug):** a `content_extent` change (same node, salt,
    /// params, upstream) MUST flip the key, otherwise two `Fit` renders of
    /// the same image+recipe at different viewport sizes collide, and the
    /// second's request silently gets served the first's stale,
    /// differently-shaped cached tile (reproduced end to end by
    /// `engine.rs`'s `distinct_viewports_do_not_collide_on_a_stale_cached_
    /// extent`).
    #[test]
    fn content_extent_change_flips_the_key() {
        let a = inputs_at("n", b"s", b"p", &[], Extent { w: 493, h: 559 });
        let b = inputs_at("n", b"s", b"p", &[], Extent { w: 936, h: 559 });
        assert_ne!(
            a, b,
            "a different requested pixel extent must never collide onto the same cache key"
        );
    }
