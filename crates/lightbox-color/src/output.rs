...
//!
//! The spec's original contract has the source always be the fixed working
//! space (ProPhoto/ROMM primaries, linear, D50); [`SourceSpace`] (E15
//! core-slice addition) makes that an explicit choice rather than a hidden
//! constant, see its doc comment for why. Each [`OutputTransform`] holds one
//! Little-CMS2 transform built for a specific source, destination space
//! **and bit depth**, plus the destination's ICC bytes for embedding. The
//! transform is cache-disabled, which makes it `Send + Sync` so E15 can apply
//! it across a rayon export pool.
...
/// The color space [`OutputTransform::apply`]'s input samples are already in
/// (E15 core-slice addition; spec §3.5/§5.7 assumes the source is always the
/// fixed linear working space, but at E15's M2 kickoff the only pixels E05's
/// `RenderTarget::Buffer` readback actually exposes are **already
/// display-encoded**, the PV1 graph's terminal node, `xform.display`, bakes
/// working → built-in-sRGB before the CPU readback ever happens; see
/// `lightbox-render/src/ng/nodes/display.rs` and the identical seam gap named
/// in `lightbox-shell`'s `panels::basic::apply_wb_pick` doc comment). Rather
/// than forking new color math in `lightbox-export`, this variant reuses the
/// exact same LCMS2 machinery with the source profile swapped to built-in
/// sRGB, so export's "convert to output space" step is still one real ICC
/// transform, not a hand-rolled matrix. `WorkingLinear` keeps the spec's
/// original contract for when a true linear export buffer lands (tracked in
/// `docs/plan/epics/E15-deviations.md`).
#[derive(Clone, Copy, PartialEq, Eq, Debug, Default)]
pub enum SourceSpace {
    /// The engine's fixed working space (ProPhoto/ROMM primaries, linear, D50)
    /// the spec's original assumption.
    #[default]
    WorkingLinear,
    /// sRGB gamma-encoded, sRGB primaries, what `RenderTarget::Buffer`
    /// readback actually yields today (E15 core-slice seam note above).
    SrgbDisplay,
}
...
impl OutputTransform {
    /// Builds an output transform (spec §3.5). **Phase D (D6).** `source`
    /// picks the input profile (E15 core-slice addition, see
    /// [`SourceSpace`]'s doc comment; the spec's original contract is
    /// `SourceSpace::WorkingLinear`); `dest`/`depth`/`intent` pick the
    /// destination profile, output pixel format, and rendering intent
    /// (rel-colorimetric carries black-point compensation).
    pub fn new(
        source: SourceSpace,
        dest: OutputSpace,
        depth: BitDepth,
        ...
        let working = match source {
            SourceSpace::WorkingLinear => crate::cms::working_linear(),
            SourceSpace::SrgbDisplay => IccProfile::srgb(),
        };
    ...
    fn every_space_builds_and_emits_valid_icc() {
        ...
        for space in spaces {
            let ot = OutputTransform::new(
                SourceSpace::WorkingLinear,
                space,
                BitDepth::Int16,
                Intent::RelColorimetric,
            )
