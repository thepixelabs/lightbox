...
impl ViewXform {
    ...
    /// outside the composited image's bounds (spec C1 AC).
    /// `#[allow(dead_code)]`: Phase F's gizmo hit-testing is the intended
    /// production consumer (screen click → image-space point); exercised
    /// here by the round-trip property tests in the meantime.
    #[allow(dead_code)]
