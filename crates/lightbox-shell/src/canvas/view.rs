...
use lightbox_render::ng::{
    CanvasFrame, Extent, OutputQuality, RenderScheduler, Roi, ViewState, Zoom,
};
...
/// unchanged by this phase. Returns `true` iff a submit was issued.
...
impl EditorCanvas {
    ...
    fn ready_ui(
        ...
        if submit_if_changed(scheduler, &mut self.last_key, entry.image, view_px, recipe) {
        ...
        let image_px = egui::vec2(entry.width as f32, entry.height as f32);
        ...
        let zoom_percent = self.zoom.resolve(fit_percent);
        if response.dragged() {
        ...
        // below and (Phase F) gizmo hit-testing.
        ...
        match tex {
            Some((texture_id, _size)) => {
                // The corners, mapped through `ViewXform` — the same
                // authority Phase F's gizmo hit-testing will use (spec
                // §6.4: "one authority" for compositing AND gizmos).
                ...
                if let Some(err) = self.progressive.error_chip() {
                    ...
                    states::chip_ui(ui, view_rect, 0, &format!("render failed: {err}"), true);
                }
            }
            None => {
...
mod tests {
    ...
    mod tier_preview_tests {
        use super::*;
        use lightbox_preview::DecodedImage;
    ...
    fn recipe_change_issues_a_new_submit_unchanged_rev_does_not() {
        ...
        assert!(
            submitted,
            "first activation of a never-seen image must submit"
        );
        ...
        let submitted = submit_if_changed(&scheduler, &mut last_key, image, out, snap(1));
        assert!(!submitted, "an unchanged rev must not resubmit");
        assert!(scheduler.wait_idle(timeout));
        assert_eq!(
        ...
        assert!(
            submitted,
            "a recipe_rev change must issue a new submit in the same frame"
        );
        assert!(scheduler.wait_idle(timeout));
        assert!(
