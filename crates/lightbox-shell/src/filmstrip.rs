...
/// Space between cells, logical points (design spec §6: `FILMSTRIP_CELL_GAP`).
const SPACING: f32 = tk::FILMSTRIP_CELL_GAP;
/// Height of the filename strip under each cell, logical points (spec §6:
/// `FILMSTRIP_META_STRIP_HEIGHT`, 16pt; the pre-reskin value was 14pt).
const LABEL_H: f32 = tk::FILMSTRIP_META_STRIP_HEIGHT;
/// Smallest useful thumbnail edge (a 48 pt strip still shows an image).
const MIN_CELL: f32 = 24.0;
/// `ELEV_0_BASE` lifted +2 tone per channel, the filmstrip's own
/// unselected-hover cell treatment (spec §5: "cell background lifts +2
/// tone"). `theme::tokens` has no standalone token for this derived value
/// since it's filmstrip-only; the arithmetic (`0x1e + 2 = 0x20`) is spelled
/// out here rather than inventing a new base color.
const CELL_HOVER_BG: egui::Color32 = egui::Color32::from_rgb(0x20, 0x20, 0x20);
...
/// the B5 drag-resize needs no extra plumbing. `forced_scroll_frac` pins
/// the scroll offset to a fraction of the scrollable range, the H2
/// `--perf-strip` sawtooth driver (the estimate only steers the sweep;
/// virtualization arithmetic still runs off the real viewport inside).
...
pub fn filmstrip_ui(
    ...
    // Top seam against the canvas above (spec §6): the filmstrip floor
    // (`ELEV_0_BASE`) is lighter than the canvas surround (`ELEV_N1_CANVAS`),
    // so the lighter side of the engraved seam is below the line.
    paint::engraved_seam_h(
        ui.painter(),
        egui::Rangef::new(strip_rect.left(), strip_rect.right()),
        strip_rect.top(),
        true,
    );
    ...
    let mut scroll_area = egui::ScrollArea::horizontal().auto_shrink(false);
    if let Some(frac) = forced_scroll_frac {
        // Scripted scroll (H2 perf capture): pin the offset to the
        // requested fraction of the scrollable range.
        let l = strip_layout(cell_size, total);
        let range = (l.total_w - ui.available_width()).max(0.0);
        scroll_area = scroll_area.horizontal_scroll_offset(frac.clamp(0.0, 1.0) * range);
    }
    ...
    // Overlay pill: re-skinned to the popup elevation + shadow (spec §11
    // recipe) instead of the previous ad-hoc darkened `extreme_bg_color`
    // this chrome floats over the strip content, so it earns the same
    // float treatment as a dropdown/tooltip.
    let pill_rect = ind_rect.union(btn_rect).expand(2.0);
    ui.painter()
        .add(paint::shadow_popup().as_shape(pill_rect, tk::RADIUS_POPUP));
    ui.painter()
        .rect_filled(pill_rect, tk::RADIUS_POPUP, tk::ELEV_3_POPUP);
    ui.put(
        ind_rect,
...
pub fn collapsed_bar_ui(
    ...
    // Same top seam as the expanded strip (spec §6), the collapsed bar
    // still sits directly under the canvas.
...
/// Paints one cell: active/selected ring, thumbnail frame (or Loading
/// shimmer / Failed placard), §6.3 badges (RAW tag, edited dot, error/
/// duplicate), clipped filename label, hover tooltip with the failure
/// reason.
///
/// Selection is two-tier (design spec §5): *active* (the single primary
/// selection) gets a 2px accent border plus a persistent restrained
/// 2-layer glow; *selected* (multi-select, not primary) gets the same 2px
/// border with no glow, a full glow per cell would be loud with many
/// selected at once; unselected *hover* is neutral-only (1px
/// `text_tertiary` border, +2-tone cell background), accent never leaks
/// into plain hover. This is layered on top of a separate, always-on
/// thumbnail-frame hairline + drop shadow (spec §6) that lifts the image
/// itself off the strip floor, independent of selection state.
#[allow(clippy::too_many_arguments)]
fn draw_cell(
    ui: &egui::Ui,
    cell_rect: egui::Rect,
    ...
    let plain_hover = is_hovered && !is_active && !is_selected;

    let cell_bg = if plain_hover {
        CELL_HOVER_BG
    } else {
        tk::ELEV_0_BASE
    };
    painter.rect_filled(cell_rect, tk::RADIUS_THUMB, cell_bg);

    let image_rect = egui::Rect::from_min_size(cell_rect.min, egui::vec2(cell_size, cell_size))
        .shrink(tk::FILMSTRIP_CELL_PADDING);

    // Thumbnail-frame lift off the strip floor (spec §6): a soft shadow
    // under the image area, independent of selection/hover state.
    painter.add(paint::shadow_thumbnail().as_shape(image_rect, tk::RADIUS_THUMB));
    ...
    // Thumbnail-frame hairline (spec §6): drawn after the image content so
    // it reads as a crisp bezel on top of the image edge, always
    // `separator_hairline` regardless of selection/hover state, the
    // constant "card" surround, distinct from the cell-level selection
    // ring below.
    painter.rect_stroke(
        image_rect,
        tk::RADIUS_THUMB,
        egui::Stroke::new(tk::STROKE_HAIRLINE, tk::SEPARATOR_HAIRLINE),
        egui::StrokeKind::Outside,
    );

    // B3 badges, painted over the image area, retoned onto the theme
    // (spec §6): backing = `ELEV_3_POPUP` at the badge alpha (stays
    // legible over arbitrary photo content), glyph = badge glyph size.
    if cell.raw {
        let tag_rect = egui::Rect::from_min_size(
            image_rect.min + egui::vec2(2.0, 2.0),
            egui::vec2(28.0, tk::FILMSTRIP_BADGE_SIZE),
        );
        painter.rect_filled(tag_rect, 2.0, badge_backing());
        painter.text(
            tag_rect.center(),
            egui::Align2::CENTER_CENTER,
            "RAW",
            egui::FontId::proportional(tk::FILMSTRIP_BADGE_GLYPH_SIZE),
            tk::TEXT_PRIMARY,
        );
    }
    if cell.is_edited {
        // Neutral `text_primary`, "edited" is a fact, not a warning, so it
        // must not borrow the accent (spec §5/§6: accent implies selection).
        let center = egui::pos2(image_rect.right() - 6.0, image_rect.top() + 6.0);
        painter.circle_filled(center, 3.0, tk::TEXT_PRIMARY);
    ...
    // Cell-level selection/hover border+glow (spec §5), drawn last/topmost
    // so it's never obscured by the image or badges. Mutually exclusive
    // states: active > selected > plain hover > rest (no border at all).
    if is_active {
        paint::glow_rect(
            &painter,
            cell_rect,
            tk::RADIUS_THUMB,
            &tk::accent_glow_filmstrip_active(),
        );
        painter.rect_stroke(
            cell_rect,
            tk::RADIUS_THUMB,
            egui::Stroke::new(tk::STROKE_SELECTION_BORDER, tk::accent()),
            egui::StrokeKind::Outside,
        );
    } else if is_selected {
        painter.rect_stroke(
        ...
        painter.rect_stroke(
            cell_rect,
            tk::RADIUS_THUMB,
            egui::Stroke::new(tk::STROKE_HAIRLINE, tk::TEXT_TERTIARY),
            egui::StrokeKind::Outside,
        );
    }

    // Filename label (spec §6/§9): `text_secondary`, 10px, ellipsis-
    // truncated via epaint's own single-row wrap+elide (no manual
    // measure-and-shrink loop) rather than the previous hard clip-rect cut.
    let label_pos = egui::pos2(cell_rect.center().x, cell_rect.bottom() - 1.0);
    let mut label_job = egui::text::LayoutJob::single_section(
        cell.filename.clone(),
        egui::TextFormat::simple(fonts::filmstrip_meta_font(), tk::TEXT_SECONDARY),
    );
    label_job.wrap.max_width = cell_rect.width();
    label_job.wrap.max_rows = 1;
    label_job.wrap.break_anywhere = true;
    let label_galley = painter.layout_job(label_job);
    let label_anchor = egui::Align2::CENTER_BOTTOM.anchor_size(label_pos, label_galley.size());
    painter.galley(label_anchor.min, label_galley, tk::TEXT_SECONDARY);

    response.clone().on_hover_text(hover.join("\n"));
}

/// `ELEV_3_POPUP` at the spec's filmstrip badge-backing opacity
/// (`FILMSTRIP_BADGE_BACKING_ALPHA`), RGB preserved, alpha scaled, the
/// same "token @ N%" convention `theme::tokens`'s own (private) `rgba()`
/// helper uses.
fn badge_backing() -> egui::Color32 {
    let c = tk::ELEV_3_POPUP;
    egui::Color32::from_rgba_unmultiplied(
        c.r(),
        c.g(),
        c.b(),
        (255.0 * tk::FILMSTRIP_BADGE_BACKING_ALPHA).round() as u8,
    )
}
...
mod tests {
    ...
    fn strip_harness(cells: Vec<StripCell>, active: Option<usize>) -> Harness<'static, StripApp> {
        ...
        // Wrapped in `themed_state`: the re-skinned cell painter goes
        // through `theme::install`-dependent chrome (glow stacks, popup
        // shadow, …), and, though today's filmstrip text stays on the
        // always-bound `Proportional` family, not a named weight role
        // wrapping here keeps this harness on the same safe pattern as
        // every other re-skinned surface rather than relying on that
        // staying true. See `theme::test_support`'s module doc for why a
        // bare kittest `Context` needs this (frame 0 installs and paints
        // nothing; `harness.run()` steps past it into the real frame).
        let mut harness = Harness::new_ui_state(
            crate::theme::test_support::themed_state(|ui, app: &mut StripApp| {
