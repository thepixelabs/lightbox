...
pub(crate) const MIGRATIONS: &[Migration] = &[
    ...
    // E08 Phase G (G2): catalog-scope settings (spec §6.7 `CatalogPrefs`)
    // a generic key/value table (see the migration file header for why KV
    // over one-column-per-setting). Consumed by `lightbox-core::prefs`.
    Migration {
        number: 6,
        name: "catalog_settings",
        sql: include_str!("../migrations/0006_catalog_settings.sql"),
        rebuilds_tables: false,
    },
    // E10 Phase D (D10): the `installed_look` registry, user-installed
    // creative-LUT packs (`.cube`/HaldCLUT), identity + provenance only
    // (spec §5.2). Files live under `<catalog>.lbdata/looks/`.
    Migration {
        number: 7,
        name: "installed_look",
        sql: include_str!("../migrations/0007_installed_look.sql"),
        rebuilds_tables: false,
    },
];
