...
// creative-LUT-pack identity/provenance DAOs (migration 0007_installed_look).
mod looks_dao;
mod migrate;
mod pages;
...
mod rawcache_dao;
mod reader;
// E08 Phase G (G2): the `catalog_settings` key/value store layer, write DAO
// on `CatalogTxn` and the matching read surface on `ReaderHandle`
// (migration 0006_catalog_settings). Values are opaque strings owned by
// `lightbox-core::prefs`.
mod settings;
mod writer;
...
pub use looks_dao::{InstalledLookRow, LookInstallOutcome, LookKind, LookSource, NewInstalledLook};
