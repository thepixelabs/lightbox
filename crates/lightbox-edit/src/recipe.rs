...
impl GlobalStages {
    /// True iff every field equals its neutral default (E10 spec A2/A5
    /// the identity-elision predicate `nodes/global` builds on: an identity
    /// `GlobalStages` builds a develop-segment graph with **zero** E10 nodes).
    pub fn is_identity(&self) -> bool {
        *self == GlobalStages::default()
    }

    /// Range-enforced clamp of every field into its documented §4.3 domain
    /// (E10 spec A2), for ingest paths that bypass `Recipe::apply`'s
    /// per-param clamp (e.g. a freshly-decoded XMP import or preset). Mirrors
    /// exactly the bounds `Recipe::set` enforces per `ParamId`.
    pub fn clamped(mut self) -> Self {
        self.white_balance.clamp();
        self.exposure = clampf(self.exposure, -5.0, 5.0);
        self.contrast = clampf(self.contrast, -100.0, 100.0);
        self.highlights = clampf(self.highlights, -100.0, 100.0);
        self.shadows = clampf(self.shadows, -100.0, 100.0);
        self.whites = clampf(self.whites, -100.0, 100.0);
        self.blacks = clampf(self.blacks, -100.0, 100.0);
        self.tone_curve.clamp_points();
        self.hsl.clamp();
        self.color_grade.clamp();
        self.vibrance = clampf(self.vibrance, -100.0, 100.0);
        self.saturation = clampf(self.saturation, -100.0, 100.0);
        self.bw.clamp();
        self.presence.clarity = clampf(self.presence.clarity, -100.0, 100.0);
        self.presence.texture = clampf(self.presence.texture, -100.0, 100.0);
        self.presence.dehaze = clampf(self.presence.dehaze, -100.0, 100.0);
        self.detail.sharpen.clamp();
        self.detail.nr.clamp();
        if let Some(lp) = &mut self.optics.lens_profile {
            lp.clamp();
        }
        self.optics.defringe = clampf(self.optics.defringe, 0.0, 100.0);
        self.optics.vignette_corr = clampf(self.optics.vignette_corr, -100.0, 100.0);
        self.effects.postcrop_vignette.clamp();
        self.effects.grain.clamp();
        if let Some(lut) = &mut self.effects.creative_lut {
            lut.clamp();
        }
        self
    }
