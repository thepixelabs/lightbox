...
/// The parametric tone curve (E10 spec §4.3 `ParametricCurve`): four
/// region-weighted sliders (highlights/lights/darks/shadows, each
/// `-100..=100`, id `0`) plus three split points partitioning `[0,1]` into
/// the four regions (id `[0.25, 0.50, 0.75]`, the LR-compatible defaults).
/// Composed with the composite [`ToneCurveSet::rgb`] point curve into one 1D
/// remap by `lightbox-render`'s `nodes::common::curve1d` (E10 task C2); it
/// carries no point data of its own and has no effect on the per-channel
/// `r`/`g`/`b` curves (an implementer decision recorded in
/// `docs/plan/epics/E10-deviations.md` Phase C: Lightroom's own Region
/// sliders likewise apply only to the master/RGB curve).
#[derive(Clone, Copy, PartialEq, Debug, Serialize, Deserialize)]
pub struct ParametricCurve {
    /// Highlights region slider, `-100..=100`.
    pub highlights: f32,
    /// Lights region slider, `-100..=100`.
    pub lights: f32,
    /// Darks region slider, `-100..=100`.
    pub darks: f32,
    /// Shadows region slider, `-100..=100`.
    pub shadows: f32,
    /// The three split points (shadow/dark, dark/light, light/highlight)
    /// strictly increasing in `(0,1)`.
    pub splits: [f32; 3],
}

/// Minimum separation kept between adjacent split points (and between the
/// outer splits and `0`/`1`) so the four parametric regions can never
/// collapse to zero width.
pub const MIN_SPLIT_GAP: f32 = 0.05;
...
impl ParametricCurve {
    /// True iff every region slider is at its neutral zero, the identity
    /// predicate `ToneCurveNode::is_identity` uses (splits are irrelevant
    /// when every slider is zero: a zero-weighted region contributes zero
    /// delta regardless of where its boundaries sit).
    pub fn is_identity(&self) -> bool {
        self.highlights == 0.0 && self.lights == 0.0 && self.darks == 0.0 && self.shadows == 0.0
    }

    pub(crate) fn clamp(&mut self) {
        self.highlights = clampf(self.highlights, -100.0, 100.0);
        self.lights = clampf(self.lights, -100.0, 100.0);
        self.darks = clampf(self.darks, -100.0, 100.0);
        self.shadows = clampf(self.shadows, -100.0, 100.0);
        for s in &mut self.splits {
            *s = clampf(*s, 0.0, 1.0);
        }
        // Enforce strictly-increasing splits with a minimum gap by pushing
        // any violator up to its predecessor + the gap (left-to-right single
        // pass), the same "clamp against the left neighbor" convention the
        // point-curve widget (`lightbox-shell::panels::curve`) uses for
        // control-point x. A degenerate all-equal input folds to evenly
        // spaced splits within `[0,1]`.
        self.splits[0] = self.splits[0].max(MIN_SPLIT_GAP);
        self.splits[1] = self.splits[1].max(self.splits[0] + MIN_SPLIT_GAP);
        self.splits[2] = self.splits[2].max(self.splits[1] + MIN_SPLIT_GAP);
        if self.splits[2] > 1.0 - MIN_SPLIT_GAP {
            // Ran off the top: fold back to the evenly-spaced default rather
            // than leaving splits crushed against 1.0 (keeps every region
            // usably wide even after repeated clamping of extreme input).
            self.splits = [0.25, 0.50, 0.75];
        }
    }
}

/// The full tone-curve set: composite RGB plus per-channel curves, edited as one
/// unit (§3.2). Neutral = all four identity (point curves) with the
/// parametric sliders at zero (E10 task C2).
...
pub struct ToneCurveSet {
    ...
    /// Blue-channel curve.
    pub b: ToneCurve,
    /// The parametric (region-slider) curve, composed with `rgb` (E10 task
    /// C2). `#[serde(default)]`: absent in an older doc's CBOR ⇒ the
    /// neutral default (forward-compat growth, spec §4.3).
    #[serde(default)]
