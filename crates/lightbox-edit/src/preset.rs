...
impl DevelopPreset {
    /// Field-wise equality **ignoring `path`** (the same preset in two stores has
    /// different paths). Used by the export→import round-trip proof (T24).
...
/// Decode a preset from a parsed [`XmpDoc`] (shared by scan + import).
fn decode_preset(
    doc: &XmpDoc,
    stem: &str,
    group: Option<String>,
