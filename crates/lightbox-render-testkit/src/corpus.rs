...
/// (re)write the golden. Shared by the A16 golden and the per-PV matrix
/// runner, and reused directly by E10's node goldens (`global.exposure`
/// et al., `tests/e10_global_basic.rs`) so every epic's golden gate speaks
/// one comparator.
pub fn compare_srgb8_to_golden(path: &Path, w: u32, h: u32, rendered: &[[u8; 4]]) -> GoldenReport {
