...
fn to_desc(row: &PreviewRow) -> Option<PreviewDesc> {
    ...
    Some(PreviewDesc {
        ...
        width: row.width,
        height: row.height,
        // One mapping, both directions, in `pyramid.rs`, including what a row
        // written by an older (or newer) build means. Never inline this again:
        // the string set is a persisted format.
        colorspace: PreviewColorspace::from_catalog_str(&row.colorspace),
