...
fn kill9_during_t2_builds_leaves_every_present_tile_readable() {
    ...
    for iteration in 0..iterations {
        ...
        // Window widened 120->400ms for load-robustness: under a CPU-starved full-suite
        // run the child could be killed before ANY T2 row committed, tripping the
        // `total_rows_seen > 0` harness check. The many short windows still land mid-build
        // (what's under test); the higher end guarantees some rows fully commit.
        std::thread::sleep(Duration::from_millis(rng.u64(15..=400)));
