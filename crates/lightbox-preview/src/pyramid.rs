...
/// Colorspace tag carried on a decoded preview (spec §5.1), **the colour
/// space the preview's pixels are actually encoded in**, and the input the
/// render engine's source→working transform is chosen from.
///
/// # From presence flag to resolved space
///
/// This enum used to be two-state (`Srgb` / `TaggedIcc`), recording only
/// *whether* a container carried an ICC profile. That made every tagged file
/// including every Display-P3 iPhone photo, fall through to the spec's
/// untagged assumption and render with sRGB primaries, i.e. over-saturated.
/// [`crate::extract::sniff_colorspace`] now resolves the embedded profile to
/// one of the named spaces below via
/// [`lightbox_color::classify_icc_source_space`].
///
/// [`PreviewColorspace::TaggedIcc`] survives, with its original meaning
/// narrowed to what it always effectively was: *there is a profile and we
/// cannot name it*. Callers treat it exactly like [`PreviewColorspace::Srgb`],
/// so an unrecognized profile renders as it always did.
///
/// # Persistence
///
/// [`PreviewColorspace::as_catalog_str`] / [`PreviewColorspace::from_catalog_str`]
/// are the single mapping to the catalog's `preview.colorspace` text column.
/// The two legacy values (`"srgb"`, `"icc"`) keep their exact old meanings, so
/// rows written by an older build still decode to the behaviour they were
/// written under, see [`PreviewColorspace::from_catalog_str`].
#[derive(Copy, Clone, PartialEq, Eq, Debug, Hash)]
#[non_exhaustive]
pub enum PreviewColorspace {
    /// sRGB, either explicitly tagged as such, or untagged (spec §5.3's
    /// "untagged ⇒ assumed sRGB").
    Srgb,
    /// Display-P3 (P3 primaries on D65, sRGB transfer). The default capture
    /// space of modern iPhones.
    DisplayP3,
    /// Adobe RGB (1998).
    AdobeRgb,
    /// ROMM RGB / ProPhoto.
    ...
    /// ITU-R BT.2020.
    Rec2020,
    /// An embedded ICC profile that is **not** one of the named spaces above
    /// (a scanner/display calibration profile, an exotic RGB space, or bytes
    /// Little-CMS2 refused). The profile itself travels with the container;
    /// this enum only records that we could not name it. Rendered as sRGB, per
    /// the spec's fallback.
    TaggedIcc,
}

impl PreviewColorspace {
    /// The value stored in the catalog's `preview.colorspace` column.
    ///
    /// `"srgb"` and `"icc"` are the two values older builds wrote and must
    /// never be repurposed, see [`PreviewColorspace::from_catalog_str`].
    pub fn as_catalog_str(self) -> &'static str {
        match self {
            PreviewColorspace::Srgb => "srgb",
            PreviewColorspace::DisplayP3 => "display-p3",
            PreviewColorspace::AdobeRgb => "adobe-rgb",
            PreviewColorspace::ProPhoto => "prophoto",
            PreviewColorspace::Rec2020 => "rec2020",
            PreviewColorspace::TaggedIcc => "icc",
        }
    }

    /// Parses the catalog's `preview.colorspace` column.
    ///
    /// # Why every unknown value maps to `TaggedIcc`
    ///
    /// The preview store is a **persistent cache that predates this enum's new
    /// variants**, and it is not versioned per-row. Two kinds of row can appear:
    ///
    /// * A row written by an older build says `"srgb"` or `"icc"`. Those keep
    ///   their original meanings exactly, `"icc"` means "tagged, unresolved",
    ///   which still renders as sRGB, so a stale row renders the way it always
    ///   did rather than acquiring a new interpretation it was never written
    ///   with. (For T0 rows the decode path does better than that: it re-reads
    ///   the profile out of the stored bytes and upgrades the tag in memory
    ///   see [`crate::decode::open_pixels`].)
    /// * A row written by a *newer* build could name a space this one has never
    ///   heard of. Falling back to `TaggedIcc` means "there is a profile and we
    ///   can't name it", the conservative reading, and the same
    ///   forward-compatible posture the store's `STORE_FORMAT_VERSION` takes.
    ///
    /// What must never happen is a stale or foreign row silently rendering as a
    /// *different* space; `TaggedIcc` is the only answer that cannot do that.
    pub fn from_catalog_str(s: &str) -> PreviewColorspace {
        match s {
            "srgb" => PreviewColorspace::Srgb,
            "display-p3" => PreviewColorspace::DisplayP3,
            "adobe-rgb" => PreviewColorspace::AdobeRgb,
            "prophoto" => PreviewColorspace::ProPhoto,
            "rec2020" => PreviewColorspace::Rec2020,
            _ => PreviewColorspace::TaggedIcc,
        }
    }
}

impl From<lightbox_color::RgbSourceSpace> for PreviewColorspace {
    fn from(space: lightbox_color::RgbSourceSpace) -> PreviewColorspace {
        use lightbox_color::RgbSourceSpace as S;
        match space {
            S::Srgb => PreviewColorspace::Srgb,
            S::DisplayP3 => PreviewColorspace::DisplayP3,
            S::AdobeRgb => PreviewColorspace::AdobeRgb,
            S::ProPhoto => PreviewColorspace::ProPhoto,
            S::Rec2020 => PreviewColorspace::Rec2020,
            // `RgbSourceSpace` is `#[non_exhaustive]`: a space added there but
            // not mirrored here must degrade to "tagged, unnamed" (⇒ sRGB),
            // never to a wrong space. Mirroring it is a deliberate edit in both
            // crates plus a catalog string, not something to infer.
            _ => PreviewColorspace::TaggedIcc,
        }
    }
}
...
mod tests {
    ...
    /// The `preview.colorspace` column is a **persisted format**: every value
    /// this build writes must read back as itself, or a cache written today
    /// renders wrong tomorrow.
    #[test]
    fn colorspace_catalog_strings_round_trip() {
        for space in [
            PreviewColorspace::Srgb,
            PreviewColorspace::DisplayP3,
            PreviewColorspace::AdobeRgb,
            PreviewColorspace::ProPhoto,
            PreviewColorspace::Rec2020,
            PreviewColorspace::TaggedIcc,
        ] {
            assert_eq!(
                PreviewColorspace::from_catalog_str(space.as_catalog_str()),
                space,
                "{space:?} did not survive the catalog column",
            );
        }
    }

    /// **Stale-cache compatibility, pinned.** `"srgb"` and `"icc"` are the only
    /// two values older builds ever wrote, and they must keep meaning exactly
    /// what they meant then, `"icc"` in particular means "tagged, unnamed",
    /// which renders as sRGB, *not* as some newly-recognized wide-gamut space.
    /// Changing either mapping silently re-interprets every preview already on
    /// a user's disk.
    #[test]
    fn legacy_catalog_values_keep_their_original_meaning() {
        assert_eq!(
            PreviewColorspace::from_catalog_str("srgb"),
            PreviewColorspace::Srgb,
        );
        assert_eq!(
            PreviewColorspace::from_catalog_str("icc"),
            PreviewColorspace::TaggedIcc,
        );
        assert_eq!(PreviewColorspace::Srgb.as_catalog_str(), "srgb");
        assert_eq!(PreviewColorspace::TaggedIcc.as_catalog_str(), "icc");
    }

    /// A row written by a *newer* build, or a corrupt one, names a space this
    /// build cannot honour. It must degrade to "tagged, unnamed" (⇒ sRGB),
    /// never to a guess, and never panic.
    #[test]
    fn an_unknown_catalog_value_degrades_to_an_unnamed_tag() {
        for s in ["", "rec2100-pq", "SRGB", "display-p3 ", "🌈", "icc\0"] {
            assert_eq!(
                PreviewColorspace::from_catalog_str(s),
                PreviewColorspace::TaggedIcc,
                "unknown column value {s:?}",
            );
        }
    }
