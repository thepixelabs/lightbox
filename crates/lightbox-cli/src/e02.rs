...
// A local `srgb_oetf` (linear → encoded) used to live here for the non-raw
// reference path. It was deleted with the double-encode fix, deliberately
// rather than left behind: nothing in this file encodes any more, the raw
// branch delegates to `InputTransform::render_reference_srgb8`, and the
// non-raw branch must not encode at all, because `decode_image` already
// returns display-referred values. Keeping an "encode to sRGB" helper in
// the one file where calling it was the bug just invites the bug back.

/// Quantizes an already display-referred `0.0..=1.0` sample to 8-bit.
fn to_u8(e: f32) -> u8 {
...
fn render_nonraw_reference(file: &Path) -> anyhow::Result<(u32, u32, Vec<u8>, String)> {
    ...
    let ch = src.channels as usize;
    let (w, h) = (src.width, src.height);
    // `decode_image` yields **display-referred** normalized f32 (see its doc
    // comment and spec §3.1), the file's own sRGB encoding, already applied.
    // Applying `srgb_oetf` here encoded it a second time and lifted every
    // non-raw reference render: this fixture's mean RGB went 120.0 -> 169.6.
    // Same defect class as the engine's missing input transform (E02
    // deviations), in the reference path instead of the graph; the raw branch
    // above is unaffected because it starts from linear camera-native data
    // and legitimately encodes once at the end.
    let mut rgba = Vec::with_capacity((w as usize) * (h as usize) * 4);
    for px in src.data.chunks_exact(ch.max(1)) {
        let r = to_u8(px[0]);
...
mod tests {
    ...
    #[test]
    fn to_u8_quantizes_display_referred_endpoints() {
        assert_eq!(to_u8(0.0), 0);
        assert_eq!(to_u8(1.0), 255);
        assert_eq!(to_u8(0.5), 128);
        // Out-of-range samples clamp rather than wrap.
        assert_eq!(to_u8(-0.5), 0);
        assert_eq!(to_u8(2.0), 255);
    }

    /// **Regression.** A non-raw reference render must round-trip an sRGB
    /// source: `decode_image` hands back *display-referred* normalized f32
    /// (its own doc; spec §3.1), i.e. the file's sRGB encoding already
    /// applied. This path used to run `srgb_oetf` over that, encoding a
    /// second time and lifting every non-raw reference render, mid-grey
    /// 128 came back as ~187, and a real fixture's mean RGB went
    /// 120.0 → 169.6. Same defect class as the engine's missing input
    /// transform, in the reference path rather than the graph.
    ///
    /// Writes a known ramp, renders it, and requires the output back
    /// within 1 LSB. Fails loudly against the double-encode.
    #[test]
    fn non_raw_reference_round_trips_an_srgb_source() {
        let dir = tempfile::tempdir().expect("tempdir");
        let path = dir.path().join("ramp.png");

        // A grey ramp spanning the range where a double encode hurts most.
        let levels: [u8; 6] = [0, 32, 64, 128, 192, 255];
        let mut rgba = Vec::with_capacity(levels.len() * 4);
        for v in levels {
            rgba.extend_from_slice(&[v, v, v, 255]);
        }
        lbx_image_compare::Rgba8Image::new(levels.len() as u32, 1, rgba)
            .expect("source image")
            .write_png(&path)
            .expect("write source png");

        let (w, h, out, _) = render_nonraw_reference(&path).expect("reference render");
        assert_eq!((w, h), (levels.len() as u32, 1));

        for (i, expected) in levels.iter().enumerate() {
            let got = out[i * 4];
            assert!(
                got.abs_diff(*expected) <= 1,
                "level {expected} came back as {got} — a double encode lifts \
                 mid-tones hard (128 -> ~187), which is the bug this guards"
            );
        }
    }
}
