...
mod tests {
    ...
    fn macos_layout() {
        let env = Env {
            home: Some(PathBuf::from("/Users/pixel")),
            appdata: None,
            xdg_data_home: None,
        };
        assert_eq!(
            build_store_dir(&env),
            PathBuf::from("/Users/pixel/Library/Application Support/Lightbox/edits.lbdata")
        );
    }
    ...
    fn windows_layout() {
        let env = Env {
            home: None,
            appdata: Some(PathBuf::from(r"C:\Users\pixel\AppData\Roaming")),
            xdg_data_home: None,
        };
        assert_eq!(
            build_store_dir(&env),
            PathBuf::from(r"C:\Users\pixel\AppData\Roaming\Lightbox\edits.lbdata")
        );
    }
    ...
    fn linux_layout_prefers_xdg_data_home() {
        let env = Env {
            home: Some(PathBuf::from("/home/pixel")),
            appdata: None,
            xdg_data_home: Some(PathBuf::from("/home/pixel/.data")),
        };
        assert_eq!(
            build_store_dir(&env),
            PathBuf::from("/home/pixel/.data/lightbox/edits.lbdata")
        );
    }
    ...
    fn linux_layout_falls_back_to_home_local_share() {
        ...
        assert_eq!(
            build_store_dir(&env),
            PathBuf::from("/home/pixel/.local/share/lightbox/edits.lbdata")
        );
    }
