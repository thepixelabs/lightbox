...
pub enum Event {
    ...
    /// `Command::Export` (E15 core slice, spec §5.10 narrowed): per-item
    /// resolution finished and `total` items are about to run.
    ExportStarted {
        /// The command's ticket.
        ticket: CommandTicket,
        /// Items in the batch (resolved items + any immediate resolution
        /// failures, e.g. a missing image, both count toward `total` so a
        /// caller correlating on `done == total` never hangs).
        total: u32,
    },
    /// One export item finished (spec §5.10, narrowed: fires once per item,
    /// not once per pipeline stage, no `ExportStage` sub-progress at
    /// core-slice scope).
    ExportProgress {
        /// The command's ticket.
        ticket: CommandTicket,
    ...
    /// The export batch finished (spec §5.10 `ExportReport`, re-used
    /// verbatim from `lightbox-export`).
    ExportFinished {
        /// The command's ticket.
        ticket: CommandTicket,
        /// Per-item ok/fail counts.
        report: lightbox_export::ExportReport,
    },
}
