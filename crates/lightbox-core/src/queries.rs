...
use lightbox_catalog::{
    CatalogCounts, EditBadge, FolderNode, ImageDetail, ImageQuery, ImageSummary, InstalledLookRow,
    Page, ReaderHandle,
};
...
impl Queries {
    ...
    /// for the D11 look-browser panel (E10 task D10, spec §5.2
    /// `all_installed_looks`).
    pub fn installed_looks(&self) -> Result<Vec<InstalledLookRow>> {
        Ok(self.reader.all_installed_looks()?)
    }
