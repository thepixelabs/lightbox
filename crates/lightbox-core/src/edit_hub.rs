...
impl EditHub {
    ...
    pub fn dispatch(&self, cmd: EditCommand) -> Result<(Vec<Event>, ChangeSet)> {
        match cmd {
            ...
            EditCommand::SeedBundledPresets { paths } => {
                let presets = self.presets()?;
                let report = presets.seed_bundled(&paths)?;
                // Import failures are per-file and non-fatal (same best-effort
                // contract as `ImportPresetFiles`), but unlike a user import
                // they also suppress the marker sweep, so say so out loud.
                if let Some(skipped) = &report.marker_sweep_skipped {
                    tracing::warn!(
                        target: "lightbox_core",
                        reason = ?skipped,
                        "starter-library reconciliation skipped; retiring nothing by marker"
                    );
                }
                tracing::info!(
                    target: "lightbox_core",
                    seeded = report.imported.len(),
                    failed = report.import_failures(),
                    retired = report.retired.len(),
                    "seeded the bundled starter preset library"
                );
                Ok((vec![], ChangeSet::default()))
            }
