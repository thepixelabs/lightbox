...
/// Rowid of an `installed_look` row (a registered creative-LUT pack, E10
/// spec §5.2). Additive per E10 Phase D (task D10), the look registry's row
/// identity, mirroring [`RawCacheEntryId`]'s pattern. The recipe-facing
/// reference (`lightbox_edit::leaves::CreativeLut::id`) is the row's
/// `content_hash` (a `String`), never this rowid, content survives
/// rename/move, a rowid would not (spec §4.8).
#[derive(
    Copy, Clone, PartialEq, Eq, Hash, PartialOrd, Ord, Debug, serde::Serialize, serde::Deserialize,
)]
pub struct LookId(pub i64);
