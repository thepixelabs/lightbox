...
//! (spec §5 T29 / §8 DoD 1), runnable on real hardware on each of the
//! three platforms:
//!
...
//! 4. runs the shell smoke (`lightbox --smoke`) — grid + loupe with the
//!    engine texture composited zero-copy on the shared device.
//!
//! Interactive browsing (scroll the grid, flip through the loupe) is the
//! human half of the drill; this command prints the checklist for it.
//! Results are recorded in `docs/plan/epics/E01-handoff.md`.
//!
...
pub fn run(root: &Path, opts: Options) -> anyhow::Result<()> {
    ...
    // -- Shell leg: grid + loupe through the engine on this hardware. -----
    if opts.no_shell {
        ...
        step(
            "shell smoke",
            t.elapsed(),
            "import → grid → loupe, engine texture zero-copy on the shared device",
        );
    }

    println!("\nEXIT DRILL PASS ({})", std::env::consts::OS);
    println!(
        "manual half (witness + record in docs/plan/epics/E01-handoff.md):\n\
         - `{}` (or --catalog <dir>): import a real shoot, scroll the grid\n\
         -   while the import runs, open the loupe (Enter/E), flip ←/→,\n\
