...
Commands:
  ...
  exit-drill [--files N] [--raw-copies N] [--no-shell]
                                   ...
                                   CLI round-trip, the E08 editor leg (edit →
                                   kill -9 → reopen → recipe restored), shell
                                   smoke
";
...
fn cmd_bundle_mac(args: &[String]) -> anyhow::Result<()> {
    let mut opts = bundle_mac::Opts::default();
    for arg in args {
        match arg.as_str() {
    ...
    bundle_mac::run(&workspace_root(), &opts).map(|_| ())
}
